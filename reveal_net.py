...
def revealing_network(container):
    with tf.variable_scope('reveal_net'):
        ...
        conv7_1 = tf.layers.conv3d(concat2,128,3,padding='same',name="23",activation=tf.nn.relu,data_format='channels_last')  
        conv7_2 = tf.layers.conv3d(conv7_1,128,3,padding='same',name="24",activation=tf.nn.relu,data_format='channels_last') 
        conv7_3 = tf.layers.conv3d(conv7_2, 128,3,padding='same',name="25",activation=tf.nn.relu,data_format='channels_last')
        

        maxpool5 = tf.layers.max_pooling3d(conv7_3, 2, strides=2, data_format='channels_last')

        conv7 = tf.layers.conv3d(conv7_3,128,3,padding='same',name="26",activation=tf.nn.relu,data_format='channels_last') 
        #print(conv7) 2x40x30x128    
        
        conv8_1 = tf.layers.conv3d(maxpool5,256,3,padding='same',name="27",activation=tf.nn.relu,data_format='channels_last')  
        conv8_2 = tf.layers.conv3d(conv8_1,256,3,padding='same',name="28",activation=tf.nn.relu,data_format='channels_last') 
        conv8_3 = tf.layers.conv3d(conv8_2,256,3,padding='same',name="29",activation=tf.nn.relu,data_format='channels_last')

        upsample3 = tf.keras.layers.UpSampling3D(size=2,  data_format='channels_last')(conv8_3)
        # print(upsample3) 4x80x60x256
        
        concat3 = tf.concat([conv7, upsample3], axis = 4, name = 'concat3')
        
        conv9_1 = tf.layers.conv3d(concat3,128,3,padding='same',name="30",activation=tf.nn.relu,data_format='channels_last')  
        conv9_2 = tf.layers.conv3d(conv9_1,128,3,padding='same',name="31",activation=tf.nn.relu,data_format='channels_last') 
        conv9_3 = tf.layers.conv3d(conv9_2,128,3,padding='same',name="32",activation=tf.nn.relu,data_format='channels_last')

        upsample4 = tf.keras.layers.UpSampling3D(size=2,  data_format='channels_last')(conv9_3)
        # print(upsample3) 8x160x120x128
        
        concat4 = tf.concat([conv2, upsample4], axis = 4, name = 'concat4')

        conv10_1 = tf.layers.conv3d(concat4,64,3,padding='same',name="33",activation=tf.nn.relu,data_format='channels_last')  
        conv10_2 = tf.layers.conv3d(conv10_1,64,3,padding='same',name="34",activation=tf.nn.relu,data_format='channels_last') 
        conv10_3 = tf.layers.conv3d(conv10_2,64,3,padding='same',name="35",activation=tf.nn.relu,data_format='channels_last')

        maxpool6 = tf.layers.max_pooling3d(conv10_3, 2, strides=2, data_format='channels_last')
        #4x80x60x64

        conv10 = tf.layers.conv3d(conv10_3,64,3,padding='same',name="36",activation=tf.nn.relu,data_format='channels_last')
 
        conv11_1 = tf.layers.conv3d(maxpool6,128,3,padding='same',name="37",activation=tf.nn.relu,data_format='channels_last')  
        conv12_2 = tf.layers.conv3d(conv11_1,128,3,padding='same',name="38",activation=tf.nn.relu,data_format='channels_last') 
        conv13_3 = tf.layers.conv3d(conv12_2,128,3,padding='same',name="39",activation=tf.nn.relu,data_format='channels_last')

        upsample5 = tf.keras.layers.UpSampling3D(size=2,  data_format='channels_last')(conv13_3)
        # print(upsample3) 8x160x120x128
        
        concat5 = tf.concat([conv10, upsample5], axis = 4, name = 'concat5')

        conv12_1 = tf.layers.conv3d(concat5,64,3,padding='same',name="40",activation=tf.nn.relu,data_format='channels_last')  
        conv12_2 = tf.layers.conv3d(conv12_1,64,3,padding='same',name="41",activation=tf.nn.relu,data_format='channels_last') 
        conv12_3 = tf.layers.conv3d(conv12_2,64,3,padding='same',name="42",activation=tf.nn.relu,data_format='channels_last')


        upsample6 = tf.keras.layers.UpSampling3D(size=(2,2,2),  data_format='channels_last')(conv12_3)
        # print(upsample3) 8x320x240x64
        
        concat6 = tf.concat([conv1, upsample6], axis = 4, name = 'concat6')

        conv13_1 = tf.layers.conv3d(concat6,32,3,padding='same',name="43",activation=tf.nn.relu,data_format='channels_last')  
        conv13_2 = tf.layers.conv3d(conv13_1,32,3,padding='same',name="44",activation=tf.nn.relu,data_format='channels_last') 
        conv13_3 = tf.layers.conv3d(conv13_2,32,3,padding='same',name="45",activation=tf.nn.relu,data_format='channels_last')

        output = tf.layers.conv3d(conv13_3,3,3,padding='same',name="46",activation=tf.nn.sigmoid,data_format='channels_last')
...
# container = tf.placeholder(shape=input_shape, dtype=tf.float32, name='cover_input')  
# reveal_network(container)